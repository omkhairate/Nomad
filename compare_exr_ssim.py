...
def _coerce_path(value: str | None, *, argument: str) -> Path | None:
    """Convert user input into a :class:`Path`.

    The original C++ utility accepted arguments in the form ``name=/path``.
    Users familiar with that interface may still pass values in that format,
    so this helper gracefully strips the ``name=`` prefix when present while
    keeping normal positional or ``--name`` arguments working unchanged.
    """
    ...
    if "=" in text:
        ...
        if normalized_key == argument.replace("_", "-"):
            text = remainder
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="PATH",
        help="Directory containing the reference EXR images (optional named form).",
    )
    parser.add_argument(
        "--test",
        dest="test_opt",
        type=str,
        metavar="PATH",
        help="Directory containing the test EXR images (optional named form).",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    reference = _coerce_path(args.reference_opt or args.reference, argument="reference")
    test = _coerce_path(args.test_opt or args.test, argument="test")

    if reference is None:
        parser.error("a reference directory must be provided (e.g. --reference path)")
    if test is None:
        parser.error("a test directory must be provided (e.g. --test path)")

    args.reference = reference
    args.test = test
    args.output = args.output.expanduser()
    if hasattr(args, "reference_opt"):
        delattr(args, "reference_opt")
    if hasattr(args, "test_opt"):
        delattr(args, "test_opt")
    return args

...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Analyze GPU timing correlation with residency onload/offload activity.")
    ...
    required = [
        "frame", "gpu_ms", "objects_onload_requested", "objects_offload_requested",
    ...
    print("=== Same-frame Pearson Correlation with gpu_ms ===")
    for col in signals:
        corr = df["gpu_ms"].corr(df[col])
        print(f"{col:28s}: {corr: .4f}")

    print("\n=== Lagged Correlation: corr(gpu_ms[t], signal[t-k]) ===")
    for col in signals:
        best_k = 0
        best = df["gpu_ms"].corr(df[col])
        for k in range(1, args.max_lag + 1):
            lag_corr = df["gpu_ms"].corr(df[col].shift(k))
            if pd.notna(lag_corr) and (pd.isna(best) or abs(lag_corr) > abs(best)):
                best = lag_corr
                best_k = k
        print(f"{col:28s}: best={best: .4f} at lag={best_k}")

    print("\n=== Segment Stats (gpu_ms) ===")

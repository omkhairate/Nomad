...
struct UniformsData {
  ...
  uint32_t restirSamplingEnabled = 0;
...
std::string Renderer::residencyStrategyName(ResidencyStrategy strategy) const {
  ...
  case ResidencyStrategy::ReSTIR:
    return "Deprecated ReSTIR (distance LOD fallback)";
...
void Renderer::updateVisibleScene() {
  ...
  case ResidencyStrategy::UnifiedScore:
    ...
    break;
  case ResidencyStrategy::ReSTIR:
    strategyName = "Deprecated ReSTIR (distance LOD fallback)";
    break;
  case ResidencyStrategy::PredictiveEnvironment:
...
void Renderer::prewarmAlwaysResidentResources() {
  ...
  const bool restirEnabled = _residencyConfig.restirSamplingEnabled;
  ...
  MTL::Texture *restirPrevSample = nullptr;
  MTL::Texture *restirPrevNormal = nullptr;
  MTL::Texture *restirPrevState = nullptr;
  MTL::Texture *restirOutSample = nullptr;
  MTL::Texture *restirOutNormal = nullptr;
  MTL::Texture *restirOutState = nullptr;
  if (restirEnabled) {
    restirPrevSample = ensureSlotReady(_restirSampleSlots[0]);
    restirPrevNormal = ensureSlotReady(_restirNormalSlots[0]);
    restirPrevState = ensureSlotReady(_restirStateSlots[0]);
    restirOutSample = ensureSlotReady(_restirSampleSlots[1]);
    restirOutNormal = ensureSlotReady(_restirNormalSlots[1]);
    restirOutState = ensureSlotReady(_restirStateSlots[1]);
  }
  ...
  bool haveAllTextures =
      ...
      normalTexture;
  if (restirEnabled) {
    haveAllTextures = haveAllTextures && restirPrevSample && restirPrevNormal &&
                      restirPrevState && restirOutSample && restirOutNormal &&
                      restirOutState;
  }
  ...
  if (useAccelerationStructureLayout) {
    if (!_pSphereBuffer || !_pSphereMaterialBuffer || !_pUniformsBuffer ||
        ...
        !_pInstanceBuffer || (restirEnabled && !_pRestirStatsBuffer)) {
      trackFrameCommandBuffer(cmd);
      cmd->commit();
      return;
    }
  ...
  if (restirEnabled) {
    ...
    compute->setTexture(nullptr, 6);
    compute->setTexture(nullptr, 7);
    compute->setTexture(nullptr, 8);
    compute->setTexture(nullptr, 9);
    compute->setTexture(nullptr, 10);
    compute->setTexture(nullptr, 11);
  }
...
const char *Renderer::textureSlotLabel(const ManagedTextureSlot &slot) const {
  ...
  if (&slot == &_restirSampleSlots[1])
    return "_restirSampleSlots[1]";
  if (&slot == &_restirNormalSlots[0])
    return "_restirNormalSlots[0]";
  if (&slot == &_restirNormalSlots[1])
    return "_restirNormalSlots[1]";
  if (&slot == &_restirStateSlots[0])
    return "_restirStateSlots[0]";
  if (&slot == &_restirStateSlots[1])
    return "_restirStateSlots[1]";
...
void Renderer::configureTextureSlot(ManagedTextureSlot &slot, NS::UInteger width,
  ...
  if (&slot == &_accumulationSlots[0] || &slot == &_accumulationSlots[1] ||
      &slot == &_sampleCountSlot || &slot == &_restirSampleSlots[0] ||
...
void Renderer::buildTextures() {
  ...
  const bool restirEnabled = _residencyConfig.restirSamplingEnabled;
  auto disableRestirSlot = [&](ManagedTextureSlot &slot) {
    releaseTextureSlot(slot);
    slot.width = 0;
    slot.height = 0;
    slot.pixelFormat = MTL::PixelFormat::PixelFormatInvalid;
    slot.usage = MTL::TextureUsage(0);
    slot.descriptorValid = false;
    slot.stagingValid = false;
  };
...
void Renderer::updateUniforms(bool cameraChanged) {
  ...
  u.restirSamplingEnabled = _residencyConfig.restirSamplingEnabled ? 1u : 0u;
...
void Renderer::draw(MTK::View *pView) {
  ...
  if (proxyRestorePending(_accumulationSlots[0]) ||
      ...
      proxyRestorePending(_restirSampleSlots[0]) ||
      proxyRestorePending(_restirSampleSlots[1]) ||
      proxyRestorePending(_restirNormalSlots[0]) ||
      proxyRestorePending(_restirNormalSlots[1]) ||
      proxyRestorePending(_restirStateSlots[0]) ||
      proxyRestorePending(_restirStateSlots[1]) ||
      historyMissing(_accumulationSlots[0]) ||
      historyMissing(_accumulationSlots[1]) ||
      historyMissing(_sampleCountSlot) ||
      historyMissing(_restirSampleSlots[0]) ||
      historyMissing(_restirSampleSlots[1]) ||
      historyMissing(_restirNormalSlots[0]) ||
  ...
  MTL::Texture *restirOutState = nullptr;
  if (restirEnabled) {
    restirPrevSample = _restirSampleSlots[0].texture;
    restirPrevNormal = _restirNormalSlots[0].texture;
    restirPrevState = _restirStateSlots[0].texture;
    restirOutSample = _restirSampleSlots[1].texture;
    restirOutNormal = _restirNormalSlots[1].texture;
    restirOutState = _restirStateSlots[1].texture;
  }
  if (allowResidency) {
    ...
    if (restirEnabled) {
      ...
      restirPrevNormal =
          requestResidentTexture(_restirNormalSlots[0], prepCmd, restoreBlit);
      restirPrevState =
          requestResidentTexture(_restirStateSlots[0], prepCmd, restoreBlit);
      restirOutSample =
          requestResidentTexture(_restirSampleSlots[1], prepCmd, restoreBlit);
      restirOutNormal =
  ...
  if (restirEnabled) {
    ...
    ensureSlotReady(_restirSampleSlots[1], restirOutSample);
    ensureSlotReady(_restirNormalSlots[1], restirOutNormal);
    ensureSlotReady(_restirStateSlots[1], restirOutState);
  }
  ...
  if (restirEnabled) {
    ...
    markSlotUsed(_restirSampleSlots[1], restirOutSample);
    markSlotUsed(_restirNormalSlots[1], restirOutNormal);
    markSlotUsed(_restirStateSlots[1], restirOutState);
  }
  ...
  if (_pPathTracePSO && !tiles.empty()) {
    ...
    while (tileIndex < tiles.size()) {
      ...
      if (restirEnabled) {
        ...
        pCompute->setTexture(nullptr, 6);
        pCompute->setTexture(nullptr, 7);
        pCompute->setTexture(nullptr, 8);
        pCompute->setTexture(nullptr, 9);
        pCompute->setTexture(nullptr, 10);
        pCompute->setTexture(nullptr, 11);
      }
...
void Renderer::updateResidency(bool forceAllToggles, bool forceFullRebuild) {
  ...
  case ResidencyStrategy::Probabilistic:
    ...
    break;
  case ResidencyStrategy::ReSTIR:
    printf("Deprecated residency strategy ReSTIR requested; falling back to distance LOD.\n");
...
void Renderer::completeFrameMetrics(MTL::CommandBuffer *pCmd) {
  ...
  if (_pRestirStatsBuffer) {
    if (auto *stats =
            static_cast<uint32_t *>(_pRestirStatsBuffer->contents())) {
